...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // Si extrajimos un correo y no hay autenticación activa en este hilo
        if (correoUsuario != null && SecurityContextHolder.getContext().getAuthentication() == null) {

            if (jwtService.validarToken(jwt, correoUsuario)) {
                // Autenticamos la petición manualmente para Spring Security
